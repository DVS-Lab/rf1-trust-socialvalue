...
The released sample has 114 participants; 113 have auditable Trust behavior and 111 enter primary computational analyses. Friends elicit higher investment. Partner-value models improve in-sample fit, but asymmetric learning predicts held-out behavior better on average, and a generic partner preference control provides a competing explanation. Ratings are **not verified as pre-task**. Frequent parameter boundaries and recovery limitations prevent interpreting a fit advantage as a uniquely social-reward mechanism.
...
Intermediate logs are in ignored `work/`. Re-running recomputes results without silently reusing cached fits. The configuration contains all statistical seeds and repetition counts. Simulated choices are stochastic; programmed outcomes are revealed only after simulated positive investments. Missing response patterns remain fixed in recovery.
