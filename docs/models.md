...
- `M5_kappa100` (formerly `M5_wide`): kappa up to 100; theta remains capped at 5.
...
Natural transforms: logistic for learning rates; exp for kappa; softplus for nonnegative social values; identity for generic partner preferences. HPreference summaries call the signed friend coefficient `preference_friend` (the audited engine internally calls it theta); this differs from H5’s nonnegative reciprocity-weighted theta. H5's bounded sensitivity uses `10*logistic(eta_theta)`.
...
CmdStan 2.40.0 via CmdStanPy; four chains, 2,000 warmup and 2,000 retained draws per chain, initial adapt_delta=.95, max_treedepth=12. All final fits use a diagonal metric. Full-data HPreference first completed 2,000 warmup iterations per chain in an interrupted diagonal run. A subsequent dense-metric attempt was much slower and was stopped before completing warmup. The final preference run reuses each original chain's last state, adapted diagonal metric and step size, then draws 2,000 fresh retained samples with adaptation disabled and a new random stream. Original pilot retained draws are discarded. This is a continuation of the same posterior target, not a refit with different priors; the two sampling segments are recorded in provenance. An initial continuation with colliding output filenames was discarded completely; the corrected continuation writes each chain to its own directory. `scripts/resume_preference_from_adaptation.py` reproduces this continuation when the archived adaptation CSVs are available; a clean pipeline run performs ordinary warmup and sampling from scratch. Runs failing diagnostics are archived and repeated at adapt_delta=.99. If mixing/ESS still fails without divergences, depth saturation or low BFMI, runs extend to 3,000 warmup and 8,000 retained draws per chain. Actual settings are recorded per run. Final diagnostics require R-hat<1.01, bulk/tail ESS≥400, no divergences or maximum-depth hits, and minimum chain BFMI>.3. Exceptions from rejected warmup proposals are recorded in raw logs; they are distinguished from retained-sample divergences. Trace figures accompany full-data fits. Raw chains/builds remain in ignored work/hierarchical.
...
The H5 training-age fit required a second mixing extension: after 8,000 draws per chain, the alpha–kappa population correlation had R-hat 1.0116 (bulk ESS 735), with zero divergences. The 8,000-draw attempt is preserved and a 16,000-draw run uses the same target and priors. Final diagnostics determine whether it is accepted.
