...
def build_gallery():
    ...
    for p in sorted((root/'figures').glob('*.png')):
        title=captions[p.name[:2]];stem=p.stem
    ...
    doc+='\n'.join(cards[:1])+f'<details><summary>Primary AICc comparison</summary><p>Mean criterion, individual best-fit frequency, and held-out performance answer different questions.</p>{table.to_html(index=False,border=0)}</details>'+'\n'.join(cards[1:])
