...
def launch_worker(entry,folder,chains,root):
    ...
    command=[sys.executable,'-u','scripts/17_parallel_checkpoint.py','--worker',entry['run']]
...
def main():
    ...
    with coordinator_lock(root):
        ...
        dispatch(checkpoint['runs'],args.jobs,lambda e:launch_worker(e,folder,args.parallel_chains,root),update)
        try:collect()
