...
def report():
    ...
    if (TABLE/'hierarchical_age_effects.csv').exists():
        a=pd.read_csv(TABLE/'hierarchical_age_effects.csv');a=a[a.run.isin(['H5_full_age','H8_full_age','H5_full_age_bounded','H5_full_quadratic','H7_full_age'])]
        parts+=['## Computational age effects\n',md_table(a[['run','parameter','quantity','term','mean','ci_low','ci_high','probability_positive']]),
                'Natural-scale curves refer to a participant at zero latent deviation from the age-dependent population mean. They are not an average over the entire population distribution. A posterior probability above zero is not a multiplicity-adjusted p value.\n',
    ...
    if (TABLE/'hierarchical_recovery_summary.csv').exists():
        ...
        'Five complete simulated datasets per condition (zero, +.35, −.35 latent theta slope per age SD), each on all 111 real ages and schedules. Generating latent means are (−1,−1.2,1), SDs (.65,.65,1); kappa/theta latent correlation is −.45. Hierarchical posterior means and 100-start theta≤10 MLEs are scored against the same truth on identical simulated choices. Population mean/SD/age slopes, individual parameters and friend-probability effects are all recovered. Five replicates give very imprecise coverage estimates and do not constitute simulation-based calibration.\n']
