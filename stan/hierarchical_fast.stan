functions {
  real rl_fast_lpmf(array[] int y, vector par, data int model_code, data int first, data int last,
                    data array[] int partner, data vector gap, data array[] int feedback,
                    data vector outcome, data vector ratings);
  vector rl_logits(vector par, int model_code, int first, int last,
                   array[] int partner, vector gap, array[] int feedback,
                   vector outcome, vector ratings) {
...
model {
  ...
  if (prior_only == 0) {
    for (i in 1:N) {
      target += rl_fast_lpmf(y | natural[i]',model_code,first[i],last[i],partner,gap,feedback,outcome,ratings[i]');
    }
  }
}
generated quantities {
  ...
  vector[N] log_lik;
  for (i in 1:N) {
    vector[last[i]-first[i]+1] logits = rl_logits(natural[i]',model_code,first[i],last[i],partner,gap,feedback,outcome,ratings[i]');
