...
stan::return_type_t<TP> rl_zero_fast_lpmf(
  ...
  for (int t=first-1;t<last;++t) {
    ...
    const double z=par[1]*dv+(zero_term?par[K-1]*zero_option[t]:0);
    const double probability=stan::math::inv_logit(z);
    ll += y[t]*z-stan::math::log1p_exp(z);
    for (int j=0;j<K;++j) {
      const double dz=par[1]*g*((1.5+bonus)*deriv[c][j]+p*db[j]+dpref[j])+(j==1?dv:0)+(zero_term && j==K-1?zero_option[t]:0);
