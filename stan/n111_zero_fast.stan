functions {
  real rl_zero_fast_lpmf(array[] int y, vector par, data int model_code, data int first, data int last,
                    data array[] int partner, data vector gap, data array[] int feedback,
                    data vector outcome, data vector ratings, data array[] int zero_option, data int zero_term);
...
model {
  ...
  if (prior_only == 0) {
    for (i in 1:N) {
      target += rl_zero_fast_lpmf(y | natural[i]',model_code,first[i],last[i],partner,gap,feedback,outcome,ratings[i]',zero_option,zero_term);
    }
  }
}
